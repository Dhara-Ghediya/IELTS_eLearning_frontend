...
def register(request):
    if request.method == "POST":
        ...
        data = {"username": uname, "email": mail, "password": pwd}
        ...
        if response.status_code == 201:
            if pwd == confirm_pass:
                ...
                messages.error(
                    request,
                    {
                        "msg": "Entered Password and Confirm Password Both are not same!",
                        "status": "success",
                    },
                )
        else:
...
def login(request):
    if request.method == "POST":
        ...
        if response.status_code == 201:
            ...
            messages.info(
                request, {"msg": "Logged in successfully!", "status": "success"}
            )
...
def profile(request):
    ...
    return render(
        request, "profile.html", context = {"title": "profile", "countries": countries}
    )
    ...
    if "std_user" in request.session.keys():
        ...
        if request.method == "POST":
            ...
            if response.status_code == 201:
                messages.success(
                    request,
                    {"msg": "Answer was submitted successfully!", "status": "success"},
                )
                return redirect("examLibrary")
            else:
                messages.error(
                    request, {"msg": "Answer submission failed!", "status": "error"}
                )
                return redirect("writing_test")
        ...
        if response.status_code == 200:
            data = response.json()
            return render(
                request, "std_writingTest.html", {"data": data, "media_url": media_url}
            )
        else:
            return render(request, "std_writingTest.html")
    else:
        messages.info(
            request, {"msg": "You cannot open this page!", "status": "warning"}
        )
        return redirect("examLibrary")
...
def listeningTest(request):
    if "std_user" in request.session.keys():
        ...
        if request.method == "POST":
            ...
            if str(ans).strip() == "":
                ans = "None"
            payload = {"question": que, "answer": ans}
            response = requests.request("POST", urls, headers = headers, data=payload)
            if response.status_code == 201:
                messages.success(
                    request,
        ...
        if response.status_code == 200:
            ...
            return render(
                request, "std_listeningTest.html", {"data": data, "media_url": audio}
            )
        else:
    ...
    if "std_user" in request.session.keys():
        ...
        if request.method == "POST":
            ...
            response = requests.request(
                "POST", urls, headers = headers, data=payload, files=files
            )
            if response.status_code == 201:
                messages.success(
                    request,
...
def myTests(request):
    ...
    return render(
        ...
        context={
            "title": "myTests",
            "writingTestData": json.dumps(writingTestData),
            "listeningTestData": json.dumps(listeningTestData),
            "readingTestData": json.dumps(readingTestData),
            "speakingTestData": json.dumps(speakingTestData),
        },
    )
